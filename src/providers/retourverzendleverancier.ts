...
export class Retourverzendleverancier extends Crud {
    constructor() {
        super(
            dict
        )
    ...
    protected async doQuery(req: Request, res: Response, next: NextFunction, options?: Dict) {
        ...
        //
        let result = await Retourtextfragmenten.getRetourleverancier(req, res, next);
        //
        ...
        res.crudData.html += await Retourtextfragmenten.getLeverancierHeader(req, res, next, '15em');
        res.crudData.html += await Retourtextfragmenten.getMidTextLeverancier(req, res, next, '5em');
        res.crudData.html += await Retourtextfragmenten.getItemsLeverancier(req, res, next, '20em');
        res.crudData.html += await Retourtextfragmenten.getBottomTextLeverancier(req, res, next, '6em');
