...
export class Bewerkingflowbewerk extends Crud {
    constructor() {
        super(
            dict
        )
...
where productlijn = '${rowproduct.LIJN}'`;
                rowsproductlijn = await db.waitQuery(res.crudConnection, sqlproductlijn);
                ...
                result += '<script>document.getElementById("b_barcode").innerHTML = draw39("B'
                    + Config.app + '-' + row.BEWERKINGSNUMMER + '-' + row.VOLGNUMMER + '",1,1);</script>';
